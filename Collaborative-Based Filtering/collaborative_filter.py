...
#Both functions read in the csv file of the movies and the ratings
def readMovies():
    movies = {}
    numMovies = 0
...
#Created the vectors needed for cosine similarity computation based on User1 and User2 ratings
...
#Generates matrix of all cosine similairties of all movies
...
'''
Arguments: 
similarityMatrix - The matrix generated by the user's ratings, holding the cosine similarity values of each movie
userRatings - the respective user's ratings of each movie
userId - id of the user to be used to find similarities to
movieId - the movie to be tested and calulated for the predicted rating
numSimilarUsers - the number of other users to be tested against 
Return: the predicted rating of the movie of the given Id based on userId
'''
...
def main():
    ...
    #Created to test multiple users
    ...
    while movieId_iterator < numMovies:
        predicted_rating = determineRating(matrix, userRatings, current_user, movieId_iterator, 15)
    ...
    top_10 = list(recommended_movies)[:10]  #Can change this to provide more recommendations if needed
    recommendations = []
    predicted_ratings = []
    ...
    accuracy = root_mean_squared_error(actual_ratings,predicted_ratings)  #Evaluation Metric to be printed to console
